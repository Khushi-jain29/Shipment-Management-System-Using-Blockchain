...
def get_table_rows(cursor, table_name):
    # Check if the table name is a valid SQLite identifier
...
def insert_into_table(cursor, tablename, cols, row):
    ...
    if not tablename.isidentifier() or not all(col.isidentifier() for col in cols):
    ...
    # Verify the number of columns and values match
    if len(cols) != len(row):
        raise ValueError("The number of columns and row values must match")

    # Create the SQL query
    columns = ', '.join([f'"{col}"' for col in cols])  # Protect column names with quotes
